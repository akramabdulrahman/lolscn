        ...
        <a href="#">
            <div class="userTicker">
                ...
                <div class="userTickerDescription">
                    ...
                    @if ($notif->riotable->in_game == 1)
                    {!! $notif->buildMsg("IN_GAME",["region"=>"{$summoner->server}","sum_name"=>" <span class='important'>$summoner->name</span>","match_type"=>config('ritocurrmatchtypes.'.$notif->riotable->match_type),"champion_name"=>config('ritochamps.'.$notif->riotable->championId)]) !!}      
                    ...
                    @if ($notif->type == "RANK")
                    {!! 
                    $notif->buildMsg($notif->type,
                    ["region"=>$summoner->server,
                    "sum_name"=> " <span class='important'>$summoner->name</span> ",
                    "downorup"=>" <span class='important'>{$notif->riotable->type}</span>",
                    "last_rank"=>str_replace(' ','_',array_values(config('ritoranks'))[array_search($notif->riotable->last_rank, array_column(config('ritoranks'), 'elo'))]['name']),
                    "new_rank"=>str_replace(' ','_',array_values(config('ritoranks'))[array_search($notif->riotable->new_rank, array_column(config('ritoranks'), 'elo'))]['name'])
                                        ])
                    !!}                 
                    @endif
